...
Die Operationalisiserung erfolgte anhand der Summenbildung der verwendeten Medien in Minuten, während denen das Kind betreut wurde (für eine Auflsitung der abgefragten Medien siehe Anhang \ref{app:Fragebogen} im Abschnitt \titleref{sec:app_mediennutzung} unter Mediennutzung). Diese Gesamtsumme $M~Summe_{Total}$ setzt sich aus den Teilsummen der Mediennutzung zusammen, an denen das Kind geschlafen hat oder wach war.  

%Formel
\begin{equation}\label{eq:MedienWach}
    M~Summe_{Wach}=\sum_{i=1}^{11} i_{Wach}
\end{equation}
\begin{equation}\label{eq:MedienGeschlafen}
    M~Summe_{Geschlafen}=\sum_{i=1}^{11} i_{Geschlafen}
\end{equation}
\begin{equation}\label{eq:MedienTotal}
    M~Summe_{Total}=\sum_{i=1}^{11} i_{Wach} + i_{Geschlafen}
\end{equation}
